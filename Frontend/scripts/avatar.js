// Vars
let domDivRockets, domRockets, domNext, domInputTeamName, domTitleTeam,teamRocket;
const letters = ["A", "B"];
let teamAvatars = [],
  teamNames = [],
  team = 0,
...
const next = function() {
  ...
  if (teamNowAvatarId != null) {
    ...
    if (teamName != "") {
      ...
      localStorage.setItem(`teamAvatarName${letters[team]}`,teamRocket)
...
const rocketSelected = function(domRocket) {
  ...
  if (domRocket.getAttribute("data-avatarSelected") == 0) {
    ...
    teamRocket=domRocket.getAttribute("name")
...
const loadAvatars = function(data) {
  ...
  for (const rocket of data) {
    output += `<div class="gallery__item gallery__item js-rockets" data-avatarId="${rocket["avatar_id"]}" name="${rocket["name"]}" data-avatarSelected="0">
