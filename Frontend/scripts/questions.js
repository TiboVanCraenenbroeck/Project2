...
const buttonclick = function() {
  ...
  btn.addEventListener("click", function() {
    ...
    // Controleer of het onderwerp is ingevult
    if (valuesubject != "") {
  ...
  const domBtnDeleteHighscore = document.querySelector(
    ".js-btn--delete-highscore"
  );
...
const buttonclickvragen = function() {
  ...
  btnvragen.addEventListener("click", function() {
    ...
    let apio =
      "https://mctproject2.azurewebsites.net/api/v1/question/" +
      onderwerpid +
      "?cookie_id=";
...
const cleanform = function() {
  ...
  let makkelijk = (document.querySelector(".js-moeilijkheid1").style.color =
    "black");
  let normaal = (document.querySelector(".js-moeilijkheid2").style.color =
    "black");
  let moeilijk = (document.querySelector(".js-moeilijkheid3").style.color =
    "black");
  var subjectinput = (document.getElementById(
    "js-input-onderwerp-vragen"
  ).value = "");
};
...
const getonderwerpen = function() {
  handleData(
    `https://mctproject2.azurewebsites.net/api/v1/subjects`,
    showonderwerpen
  );
};
...
const buttondeletequestion = function() {
  ...
  for (const dombtn of btn) {
    dombtn.addEventListener("click", async function() {
      ...
      let preparedapi =
        api + onderwerpid + "/" + questionId + "?cookie_id=" + decid;
...
const getDataFromInputfields = function(vraagId) {
  ...
  let radiobtna = document.querySelector(`.js-radioa-question--${vraagId}`)
    .checked;
  let radiobtnb = document.querySelector(`.js-radiob-question--${vraagId}`)
    .checked;
  let radiobtnc = document.querySelector(`.js-radioc-question--${vraagId}`)
    .checked;
  let radiobtnd = document.querySelector(`.js-radiod-question--${vraagId}`)
    .checked;
  ...
  let moeilijkheidsgraad = document
    .querySelector(`.js-level--${vraagId}`)
...
const getMoeilijkheidsgraad = function(vraagId) {
  ...
  const domBtnMoeilijkheidsgraad = document
    .querySelector(`.js-input-question--${vraagId}`)
    .parentElement.querySelector(`.radio-input--${vraagId}:checked`).value;
...
const btnMoeilijkheidsgraad = function() {
  ...
  const domBtnMoeilijkheidsgraden = document.querySelectorAll(
    ".js-moeilijkheidsgraad"
  );
...
const resetBtnsMoeilijkheidsgraad = function(questionId) {
  ...
  const domBtnMoeilijkheidsGraadAll = document.querySelectorAll(
    `.js-moeilijkheidsgraad--${questionId}`
  );
...
const checkUserClickedOnBtnLevel = function(domBtnMoeilijkheidsgraad) {
  ...
  document
    .querySelector(`.js-level--${questionId}`)
    .setAttribute("data-level", domBtnMoeilijkheidsgraad.innerText);
};
  ...
  for (let i = 0; i < datavragen.length; i++) {
    ...
    vragenHTML += `<div class="c-form-delete js-form-delete">
        <input class="c-input-vragen js-input-question--${
          quizdata.questionid
        }" value="${quizdata.question}" id="js-vraag"></input>
        
        <label class="c-lbl-antwoorden c-margin">Antwoorden:</label>
    
        <input class="c-radio-a c-radiobtn-option js-radioa-question--${
          quizdata.questionid
        }" ${
      quizdata.answers[0].correct ? "checked" : ""
    } id="js-rbtn-a${i}" type="radio" name="vragen-${i}" value="a"><br>
                
        <input class="c-radio-b c-radiobtn-option js-radiob-question--${
          quizdata.questionid
        }" ${
      quizdata.answers[1].correct ? "checked" : ""
    } id="js-rbtn-b${i}" type="radio" name="vragen-${i}" value="b"><br>
            
        <input class="c-radio-c c-radiobtn-option js-radioc-question--${
          quizdata.questionid
        }" ${
      quizdata.answers[2].correct ? "checked" : ""
    } id="js-rbtn-c${i}" type="radio" name="vragen-${i}" value="c"><br>
        
        <input class="c-radio-d c-radiobtn-option js-radiod-question--${
          quizdata.questionid
        }" ${
      quizdata.answers[3].correct ? "checked" : ""
    } id="js-rbtn-d${i}" type="radio" name="vragen-${i}" value="d">  
        <input class="c-input-a c-input-style js-input-answera--${
          quizdata.questionid
        }" value="${quizdata.answers[0].answer}" id="js-antwoorda"></input>
        <input class="c-input-b c-input-style js-input-answerb--${
          quizdata.questionid
        }" value="${quizdata.answers[1].answer}" id="js-antwoordb"></input>
        <input class="c-input-c c-input-style js-input-answerc--${
          quizdata.questionid
        }" value="${quizdata.answers[2].answer}" id="js-antwoordc"></input>
        <input class="c-input-d c-input-style js-input-answerd--${
          quizdata.questionid
        }" value="${quizdata.answers[3].answer}" id="js-antwoordd"></input>
        <div class="c-div-level c-margin">
        <label class="c-lbl-level js-level--${
          quizdata.questionid
        }" data-level="${quizdata.difficult}">Moeilijkheid: </label>`;
...
<label for="dif-3" class="c-lbl-3 js-moeilijkheid3 c-moeilijkheidsgraad__selected js-moeilijkheidsgraad-question--change js-moeilijkheidsgraad--${quizdata.questionid}" data-question-id="${quizdata.questionid}" data-level-selected="false">2</label>`;
  ...
  let domBtnsMoeilijkheidsgraden = document.querySelectorAll(
    ".js-moeilijkheidsgraad-question--change"
  );
  // Controleer of iemand op de knop geklikt heeft
...
const buttonclickallevragen = function() {
  ...
  btn.addEventListener("click", async function() {
    ...
    // Controleer of er een onderwerp geselecteerd heeft
    if (valuesubject != "") {
      for (var quiz_id in quiz_ids) {
        if (!quiz_ids.hasOwnProperty(quiz_id)) continue;
        const everything = quiz_ids[quiz_id];
